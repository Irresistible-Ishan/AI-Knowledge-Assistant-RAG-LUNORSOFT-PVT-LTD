...
The user is a student, and your task is to act as the knowledge distributor. 
Do not make up data; ground your answers from the given embedding context. 
If no context is given, do not make up data unless you are 100% sure. 
If you have no valid data, tell the user directly about it.

Work efficiently, teach cleanly and write your explanations in proper formatting. 
Do not assume the user knows the details; be very clear and specific.
...
qa_template = PromptTemplate(mainPrompt)
...
class KnowledgeAgent:
    ...
    def ask(self, query: str ) :
        ...
        if recent_history :
            chat_context = f"Previous Chat:\n{recent_history}\n\nCurrent Question: {query}"
        else:
            chat_context = query           
        for llm in self.llms:
            try:
                if not self.mode or self.data is None :
                    res = llm.complete(chat_context)
                    ans_text = str(res)
                    self.history_list.append(f"User: {query}")
                    self.history_list.append(f"Assistant: {ans_text}")
                    return ans_text, [], llm.model
                    
                elif self.mode == "direct" :
                    final_prompt = qa_template.format(
                        context_str=self.data,
                        query_str=chat_context
                    )
                    ...
                    return ans_text, refs, llm.model
                    ...
                    final_prompt = qa_template.format(context_str=context_str, query_str=chat_context)                
                    res = llm.complete(final_prompt)  
                    ans_text = str(res)
                    self.history_list.append(f"User: {query}")
                    self.history_list.append(f"Assistant: {ans_text}")                    
